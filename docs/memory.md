...
## 2026-08-13 09:09:13 KST

### 배포 사이트를 소유자 전용으로 전환

- Sites 접근 방식을 공개에서 소유자 전용으로 변경했다.
- 같은 ChatGPT 소유자 계정으로 로그인한 PC와 휴대폰에서만 사이트를 열 수 있으며, Supabase 업무 데이터 보호와 자동 동기화 방식은 그대로 유지한다.
- 접근 상태를 다시 확인해 소유자 1명만 허용되고 추가 사용자, 외부 방문자, 사용자 그룹이 모두 0명임을 확인했다.
...
## 2026-08-13 15:52:17 KST

### 휴대폰 접속을 위한 공개 화면·비공개 데이터 방식 적용

- 휴대폰에서 Sites 소유자 로그인 화면 없이 접속할 수 있도록 사이트 접근 방식을 소유자 전용에서 공개로 변경했다.
- 사이트 화면은 주소를 아는 사람이 열 수 있지만, 실제 업무 데이터는 Supabase 인증 사용자 ID와 일치하는 행만 읽고 변경할 수 있도록 기존 RLS 정책을 유지했다.
- 로그인하지 않은 요청으로 업무 목록을 조회해 빈 목록만 반환되는 것을 확인했고, 브라우저 코드에 관리자용 비밀 키가 없음을 확인했다.
